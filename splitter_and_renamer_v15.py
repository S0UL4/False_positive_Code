...
import time
from custom_ask import ask_custom_yes_no_cancel
import tkinter.simpledialog


#GUI Params
root = tk.Tk()
root.title('K-1 Unmask, Split and Rename v15')
...
def get_name_of_k1(page,file_name):
    ...
    for l in page.get_text_blocks(sort=True):
        ...
        if("For calendar year" in l[4]):
            year=l[4][l[4].find('20'):l[4].find('20')+4]
        ...
        for page in doc:
            ...
            if(Form_8879_PE in selector):
                ...
                if(not(EIN) and proceed_or_not==False):
                    temp_EIN=tkinter.simpledialog.askstring(title = "EIN Input" , prompt = "Please enter the EIN in format AA-BBBBBBB for file "+str(file_name) )
                    try:
                        if(not(len(temp_EIN)==10)):
                            temp_EIN="ERRLEN"  
                        
                        elif(not(temp_EIN[:2].isnumeric())):
                            temp_EIN="ERRFP"  #FP for saying First part AA

                        elif(not(temp_EIN[3:].isnumeric())):
                            temp_EIN="ERRLP"  #FP for saying seconond part BBBBBBB
                    ...
                    # # print(EIN)
                    # proceed_or_not=ask_custom_yes_no_cancel(rt=root,
                    #                 message="EIN on 8879 is Masked for %s file \nDo you want to add EIN ?"%(file_name),
                    #                 title="K-1 Unmask, Split and Rename v13"
                    #             )
                    #print(proceed_or_not)
                    
                    if("ERRLEN" in temp_EIN):
                        showinfo(
                            title='Split aborted for file '+str(file_name),
                            message="Check the EIN length"
                        )
                        return 0

                    if("ERRFP" in temp_EIN):
                        showinfo(
                            title='Split aborted for file '+str(file_name),
                            message="EIN Must be numeric and follow AA-BBBBBBB"
                        )
                        return 0

                    if("ERRLP" in temp_EIN):
                        showinfo(
                            title='Split aborted for file '+str(file_name),
                            message="EIN Must be numeric and follow AA-BBBBBBB"
                        )
                    ...
                    if(temp_EIN):
                        ...
                        showinfo(
                            title='Split aborted for file '+str(file_name),
                            message="Please give an EIN to continue !"
                        )
                        return 0                        
                    # if("cancel" in proceed_or_not or "abort" in proceed_or_not):
                    #     showinfo(
                    #         title='Info',
                    #         message="Split aborted for file "+str(file_name)
                    #     )
                    #     return 0
                    
                    # if("continue" in proceed_or_not):
                    #     pass
                    # else:
                    #     EIN=proceed_or_not
            ...
            if(form_k1 in selector):
                # abort split job if the page 8879 is already missing 
                if(not(found_8879_page_flag)):
                    showerror(
                            title='Missing 8879',
                            message="8879 Page is missing for %s\nAbort split."%file_name
                    )
                    return 0
                ...
                number= pick_next_pages(doc,page) if not(pick_next_pages(doc,page)==None) else 0  # rassembling different next pages that are linked to previous k-1
